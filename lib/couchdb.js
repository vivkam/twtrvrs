	...
	db.save(
		'_design/dm',
		{
			sender : {
				map : function (doc) {
					if (doc.type === 'dm') {
						emit(doc.sender_screen_name, doc);
					}
				},
				reduce : function (key, value) {
					return value.length;
			...
			recipient : {
				map : function (doc) {
					if (doc.type === 'dm') {
						emit(doc.recipient_screen_name, doc);
					}
				},
				reduce : function (key, value) {
					return value.length;
...
function createBackupQueueViews (callback) {
	...
	db.save(
		'_design/backup',
		{
			tweets : {
				map : function (doc) {
					if (doc.type === 'backup' && doc.backupType === 'tweet') {
						emit(doc.id_str, doc);
					}
				}
			},
			retweets : {
				map : function (doc) {
					if (doc.type === 'backup' && doc.backupType === 'retweet') {
						emit(doc.id_str, doc);
					}
				}
			},
			users : {
				map : function (doc) {
					if (doc.type === 'backup' && doc.backupType === 'user') {
						emit(doc.screen_name, doc);
					}
				}
			},
			media : {
				map : function (doc) {
					if (doc.type === 'backup' && doc.backupType === 'media') {
						emit(doc.id_str, doc);
					}
				}
			}
