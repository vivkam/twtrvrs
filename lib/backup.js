			...
			soopahviv.get(
				...
				function (tweet) {
					...
					if (tweet instanceof Error) {
						console.error('error fetching tweet ' + id + ': ' + util.inspect(tweet));
					} else {
						user = tweet.user;
						tweet.user = twitUtil.getTrimmedUser(user);
						db.save(id, tweet, function (error, response) {
							if (error) {
								console.error('error saving tweet ' + id + ': ' + error.error);
							}
						});
...
function saveOrUpdateUser (user) {
	...
	db.save(_id, user, function (error, response) {
		if (error && error.error !== 'conflict') {
			console.error('error saving user ' + _id + ': ' + error.error);
		}
	});
}
...
function saveToBackupQueue (entity, backupType) {
	...
	function saveBackupDocument (_id) {
		db.save(_id, doc, function (error, response) {
			if (error && error.error !== 'conflict') {
				console.error('error saving id ' + id_str + ' to backup queue: ' + error.error);
			}
		});
	}
	...
	if (backupType === 'retweet') {
		...
		db.get(
			twitUtil.getIdForType(backupType, doc.id_str),
