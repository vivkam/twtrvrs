 ...
 *
 * callback object:
 * - result.returnCount: number of tweets returned from twitter
 * - result.saveCount: number of tweets saved
 * - result.minId: id of oldest tweet saved
 * - result.parameters: paremters sent with api request
 * - result.error: error object if there was an error
 */
 ...
 * Document structure: { _id : id , needsBackup : true}
 *
 * saveToBackupQueue(id)
 */
function saveToBackupQueue (_id, type) {
	db.put('backup:' + type + ':' + _id, { needsBackup : true }, function (error, response) {
		if (error && error.error !== 'conflict') {
			console.error('error saving id ' + _id + ' for future backup: ' + util.inspect(error));
		}
	});
}