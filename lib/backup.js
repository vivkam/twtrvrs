	...
	return function (response) {
		if (response.error) {
			if (response.error instanceof Error) {
				console.error(type + ' twitter api error: ' + response.error.toString());
			} else {
				console.error(type + ' database error: ' + util.inspect(response.error));
			}
		}
...
function backupFavorites (callback) {
	...
	backupFromTwitter('/favorites/list.json', parameters, callback);
}
			...
			soopahviv.get(
				...
				function (tweet) {
					...
					if (tweet instanceof Error) {
						console.error('error fetching tweet ' + id + ': ' + tweet.toString());
					} else {
						user = tweet.user;
						tweet.user = twitUtil.getTrimmedUser(user);
						db.save(id, tweet, function (error, response) {
							if (error) {
								console.error('error saving tweet ' + id + ': ' + util.inspect(error));
							}
						});
