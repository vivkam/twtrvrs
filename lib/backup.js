	...
	return function (response) {
		if (response.error) {
			if (response.error instanceof Error) {
				console.error('error fetching ' + type + ': ' + response.error.toString());
			} else {
				console.error('error saving ' + type + ' tweet: ' + util.inspect(response.error));
			}
...
function backupRelatedEntities (tweet) {
	...
	if (tweet.entities && tweet.entities.media) {
		tweet.entities.media.forEach(function (media) {
			saveToBackupQueue(media.id_str, 'media', media.media_url);
		});
	}
}
 ...
 * Document structure: { _id : "backup:{id}" , type : {backupType} , id_str : {id} , url : {url}}
 *
 * saveToBackupQueue(id_str, backupType, [url])
 *
 * Accepted types: tweet, retweet, user, media
 */
function saveToBackupQueue (id_str, backupType, url) {
	...
	function saveBackupDocument () {
		...
		db.save(backup_id, doc, function (error, response) {
			if (error && error.error !== 'conflict') {
	...
	if (backupType === 'retweet') {
		...
		check_id = twitUtil.getIdForType(backupType, id_str);
